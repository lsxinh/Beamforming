...
load(['/data/geophys/scratch/jn6g09/CASCADIADATA/Event_2013_018/LDH_processedbmIGW/LDH.2013.18'],'-mat');
...
for ic= [4 5 6]
    ...
    for iday=2:Ntime%:2 %2:Ntime %270:330 %300 %10:1:Ntime-5 %1:Ntime% 40 %1:1:210 %Ntime-5 %1:Nday

      %    tre=double(squeeze(mean(beam(:,ic+[-2:1:2],:,iday+[-5:5]),2)));
     %   tre=double(squeeze(mean(beam(:,ic+[-5:1:5],:,iday+[-1:1]),2)));
        tre=double(squeeze(mean(beam(:,ic+[-0:1:0],:,iday),2)));
 ...
 %       subplot(4,4,isub)
         hold off
        h=polarpeter([0 2*pi], [1 25]); axis('ij'), delete(h), hold on, view([-90 90])
        ...
        polarpeter(pi/4*[1 1]'*[1:8] ,[1 25]'*ones(1,8),'w')
