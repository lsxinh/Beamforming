...
for kk=1:length(ev); %loop over Events
    ...
    for p=1:nfiles %loop over stations
        ...
        pp=9; %data split into 2048 second timeseries   
        frq=Fe/2*([1:2^(pp-1)]-1)/2^(pp-1); %set up frequency axis
        %frqtest=Fe/2*linspace(0,1,2^11/2+1);
        Ismall=2^pp;  %length of each sub sample
        display(['Ismall' num2str(Ismall)])
        ipick=1:Ismall;
        Nsub=floor(size(seis,1)/2^pp); %number of sub samples
        Nsample(imonth,istacur,iyr-1995)=Nsub;
        seissmall=zeros(2^pp,Ndays,Nsub);
        ...
        %fft each sub sample
        fseis=fft(seissmall, 2^pp); %plot(abs(fseis(:,1,1)))
